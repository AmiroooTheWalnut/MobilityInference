...
class MonthData:
    def __init__(self, data):
        self.pOIs = torch.tensor(data[0].values).cuda()
        self.needs = data[1]
        self.pOIShops = torch.tensor(data[4].iloc[:, 1:].values).cuda()
        ...
        self.needsTensor = torch.tensor(self.needs.values).cuda()
        self.isFirst = data[3]
        ...
        self.populationCBG = torch.tensor(data[10].values).cuda()
        self.NCBG = data[10].shape[0]
        self.populationNum = ((self.populationCBG * self.N).flatten()).cuda()
