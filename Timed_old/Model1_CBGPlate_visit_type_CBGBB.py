    ...
    def run(self,numParticles,lr,elboType,alpha,i,retVals):
        ...
        def model(data):
            ...
            with pyro.plate('observe_data'):
                shopVisitsObs = pyro.sample("S_Shop", dist.Poisson(torch.sum(torch.transpose(torch.div(data[0].alpha_paramShop.repeat([data[0].NCBG, 1]), data[0].alpha_paramShop.repeat([data[0].NCBG, 1]) + data[0].beta_paramShop[g].repeat([data[0].NCBG, 1])), 0, 1) * data[0].BBNSh)), obs=trainShopObs*gapVal)
                schoolVisitsObs = pyro.sample("S_School", dist.Poisson(torch.sum(torch.transpose(torch.div(data[0].alpha_paramSchool.repeat([data[0].NCBG, 1]), data[0].alpha_paramSchool.repeat([data[0].NCBG, 1]) + data[0].beta_paramSchool[g].repeat([data[0].NCBG, 1])), 0, 1) * data[0].BBNSch)), obs=trainSchoolObs*gapVal)
                religionVisitsObs = pyro.sample("S_Religion", dist.Poisson(torch.sum(torch.transpose(torch.div(data[0].alpha_paramReligion.repeat([data[0].NCBG, 1]), data[0].alpha_paramReligion.repeat([data[0].NCBG, 1]) + data[0].beta_paramReligion[g].repeat([data[0].NCBG, 1])), 0, 1) * data[0].BBNRel)), obs=trainRelObs*gapVal)
            ...
            for i in range(data[0].globalError.shape[0]):
                if data[0].globalError[i] == 0:
                    ...
                    data[0].globalError[i] = torch.absolute(shopAnalytical - data[0].pOIs[0, 1]*gapVal) + torch.absolute(schoolAnalytical - data[0].pOIs[1, 1]*gapVal) + torch.absolute(religionAnalytical - data[0].pOIs[2, 1]*gapVal)
